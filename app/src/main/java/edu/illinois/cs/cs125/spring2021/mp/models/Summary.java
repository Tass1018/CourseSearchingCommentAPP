...
public class Summary implements SortedListAdapter.ViewModel {
   ...
   * @param courses
   * @param text
   * @return the courses for this Summary
   */
  public static List<Summary> filter(
    ...
    for (Summary course : courses) {
      String str  = (course.department + " " + course.number + ": " + course.title).toLowerCase();
      String allLowerCase = text.toLowerCase();
      if (str.contains(allLowerCase)) {
        lists.add(course);
      }
    }
    return lists;
